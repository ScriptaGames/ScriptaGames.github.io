...
var PlayState = function (_Phaser$State) {
    ...
    _createClass(PlayState, [{
        ...
        value: function drawInitialScene() {
            ...
            skyTween.to({ y: -this.sprites.sky.height }, 40 * Phaser.Timer.SECOND, Phaser.Easing.Linear.None, true, 0, -1);
            skyTween.onUpdateCallback(function () {
                var y = Math.floor(Math.abs(_this5.sprites.sky.position.y + 1 / 8 * _this5.game.world.height) % _this5.sprites.sky.height);
                // const y = Math.floor(Math.abs(-this.sprites.sky.position.y));
                var color = _this5.skyBMD.getPixel32(20, y);
                // console.log(`maybe it is ${y}, ${color}`);
                _this5.sprites.mountain.tint = color;
                _this5.sprites.cabin.tint = color;
                _this5.sprites.you.tint = color;
                _this5.sprites.generator.tint = color;
                _this5.sprites.cupboard.tint = color;
                _this5.sprites.fuel.tint = color;
                _this5.sprites.heater.tint = color;
            }, this);
            var sky2Tween = this.game.add.tween(this.sprites.sky2);
            sky2Tween.to({ y: 0 }, 40 * Phaser.Timer.SECOND, Phaser.Easing.Linear.None, true, 0, -1);
            ...
            this.meterNames.forEach(function (meterName) {
                var sprite = _this5.game.add.sprite(0, 0, 'meter');
                _this5.sprites[meterName + 'Meter'] = sprite;
                sprite.anchor.set(1, 1);
                sprite.alpha = 0.5;
                _this5.meterGroup.addChild(sprite);
            });
            // add meter labels
            var meterNameMap = {
                fuelInUse: 'GENER',
            ...
            this.meterNames.forEach(function (meterName) {
                var text = _this5.game.add.text(0, 0, (meterNameMap[meterName] || meterName).toUpperCase(), {
                    // https://photonstorm.github.io/phaser-ce/Phaser.Text.html
                    font: 'bold 16px monospace',
                    fill: '#4793D1'
                });
                text.anchor.set(1, 1);
                _this5.meterGroup.addChild(text);
            });
            this.meterGroup.align(this.meterNames.length, 2, 70, 30, Phaser.BOTTOM_CENTER);
            this.meterGroup.position.set(this.game.world.width - 60 - this.meterGroup.width, this.game.world.height - 60 - this.meterGroup.height
            ...
            this.meterNames.forEach(function (meterName) {
                ...
                sprite.anchor.set(1, 1);
                sprite.height = 100;
                var mainMeter = _this5.sprites[meterName + 'Meter'];
                sprite.position.copyFrom(mainMeter);
                sprite.position.add(_this5.meterGroup.position.x, _this5.meterGroup.position.y);
                sprite.moveDown();
                // sprite.alpha = 0.5;
            });
            ...
            [''].forEach(function (num) {
                var sprite = _this5.sprites['mountain' + num];
                sprite.anchor.set(1, 1);
                var blurOffset = 0;
                if (num >= 1) {
                    blurOffset = 10 * num;
                }
                sprite.position.set(_this5.game.world.width + blurOffset, _this5.game.world.height + blurOffset);
            });
